...
app.post('/user/auth', (req, res) => {
	...
	if (!password || password == "") {
		res.redirect(
		  `${proxy_path}/login/#${encodeURIComponent(errorMessages.NO_PASSWORD)}`
		);
		return;
	}
	if (getUser(userid) === false) {
		res.redirect(
		  `${proxy_path}/login/#${encodeURIComponent(errorMessages.USER_NOT_FOUND)}`
		);
		return;
	}
	  if (
		computeHash(password, getUser(userid).salt) != getUser(userid).passwordHash
	  ) {
		res.redirect(
		  `${proxy_path}/login/#${encodeURIComponent(
...
app.post('/user', (req, res) => {
  ...
  if (!userid || userid == "") {
    res.redirect(
      `${proxy_path}/registration/#${encodeURIComponent(errorMessages.NO_USER)}`
    );
    return;
  }
  ...
  if (!password || password == "") {
    res.redirect(
      `${proxy_path}/registration/#${encodeURIComponent(
        errorMessages.NO_PASSWORD
      )}`
    );
    return;
  }
  ...
  if (!name || name == "") {
    res.redirect(
      `${proxy_path}/registration/#${encodeURIComponent(errorMessages.NO_NAME)}`
    );
    return;
  }
